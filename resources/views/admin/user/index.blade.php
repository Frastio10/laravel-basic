				...
				<tbody>
					@foreach($users as $key => $user)
						<tr>
							...
							<td>{{ $user->hobby }}</td>
							<td>
								<a href="{{ route('admin.user.edit',$user->id) }}" class="btn btn-sm float-left btn-warning mr-1 text-white">Edit</a>
								<form action="{{ route('admin.user.delete') }}" class="float-left" method="POST">
									@csrf
									{{ method_field('DELETE') }}
									<input type="hidden" name="id" value="{{ $user->id }}">
									<button href="{{ route('admin.user.delete') }}" type="submit" class="btn btn-sm btn-danger text-white">Delete</button>
